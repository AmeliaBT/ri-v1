...
const bcrypt = require('bcryptjs'); // bcryptjs bcrypt
const saltRounds = 10;
// session
...
// connection
const promise_connection = mongoose.connect(url, { useUnifiedTopology: true, useNewUrlParser: true }); //, { 	useMongoClient: true }
...
//offset reportID by 
  reportModel.find({}, (err, docs) => {  
   // request.nd =(docs.length +100000200 ).toString(); 
     request.nd =(docs.length +100000300 ).toString(); 
...
lwo: request.body["lwo"], //photo 1 string
...
uwo: request.body["uwo"] //photo 2 file 
};
...
// PAGE: Reports ;  path: list-all2 ;  component: RIlistAll2
//on load show 50 pages
app.post("/get-all-users-reports-p", function(request, response) {
  
       reportModel.find({}, (err, docs) => {
          if(err) throw err;
         let pageLength = 50;
         let docs2= docs.sort({cwo:  -1,reportID:  -1 });
           let docStart =docs2.length -1;
         let docEnd =docs2.length - pageLength ;             
          let reports = []   ;          
          for(let i = docStart; i > docEnd -1; i--) {            
            reports.push(docs2[i]);              
            if(i == docEnd) response.json({reports: reports});        
            
          }
       })

  //sort({ reportID: 'asc', daterec: -1 });

});
// component: FilterAP 
app.post("/count-documents", function(request, response) {

  reportModel.find({}, (err, docs) => {  
   // request.nd =(docs.length +100000200 ).toString(); 
    let n= docs.length; 
    if(!err) { response.json({totalDocs: n});  } 
    else { console.log("ERROR!: ", err);}  
 }); 
...
// PAGE: Reports ;  path: list-all2 ;  component: RIlistAll2
/*two options support pagination: limit and skip
skip() will return a certain number of results after a certain number of documents.
limit() is used to specify the maximum number of results to be returned.
MyModel.find({ name: /john/i }, null, { limit=5, skip: 10 })
*/

app.post("/create-filtered-table-page", function(request, response) {
  let currentPage= request.body.page;
  
  let pageLength = 50;
  // skip qty: currentPage*pageLength
...
dateN.setDate(dateN.getDate() -365);
  ...
  //______________________________________________________
    ...
    cwo: dateSpan 
       ...
       ).sort({cwo:  -1,reportID:  -1 }).skip((currentPage-1)*pageLength ).limit(pageLength).exec(                      
                   (err, doc) => {     
      if (err) throw err;   
           response.json(doc);  
          }
  ...
  // daterec  reportID: skip(currentPage*pageLength -1).
  //,  null, {limit:pageLength,skip:currentPage*pageLength }, 
 /* 
   reportModel.find({ 
    reportID: new RegExp(regreportID, 'ig'), 
     Gwo: new RegExp(regGwo, 'ig'), 
